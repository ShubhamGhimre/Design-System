...
export interface SignUpSplitScreenProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'onSubmit'> {
...
export function SignUpSplitScreen({
  className,
  onSubmit,
  onToggleMode,
  brandSlot,
        ...
        {imageSlot ?? (
          <div className="flex h-full items-center justify-center bg-gradient-to-br from-primary/20 to-primary/5 p-12">
            <div className="text-center">
              <h2 className="text-3xl font-bold text-foreground">Join Us Today</h2>
              <p className="mt-2 text-muted-foreground">Create an account and get started.</p>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            <div className="space-y-2">
              <Label htmlFor="sus-email">Full Name</Label>
              <Input id="sus-name" placeholder="Jane Doe" value={name} onChange={(e) => { setName(e.target.value); setFieldErrors((p) => ({ ...p, name: '' })) }} aria-invalid={!!fieldErrors.name} />
              {fieldErrors.name ? <p className="text-xs text-destructive">{fieldErrors.name}</p> : null}
            </div>
            <div className="space-y-2">
              <Label htmlFor="sus-email">Email</Label>
              <Input id="sus-email" type="email" placeholder="hello@example.com" value={email} onChange={(e) => { setEmail(e.target.value); setFieldErrors((p) => ({ ...p, email: '' })) }} aria-invalid={!!fieldErrors.email} />
              {fieldErrors.email ? <p className="text-xs text-destructive">{fieldErrors.email}</p> : null}
            </div>
            <div className="space-y-2">
              <Label htmlFor="sus-password">Password</Label>
              <div className="relative">
                <Input id="sus-password" type={showPassword ? 'text' : 'password'} placeholder="Min. 8 characters" value={password} onChange={(e) => { setPassword(e.target.value); setFieldErrors((p) => ({ ...p, password: '' })) }} aria-invalid={!!fieldErrors.password} />
